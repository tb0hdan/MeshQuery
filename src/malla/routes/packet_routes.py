...
def get_packet_details(packet_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        # Convert binary data to safe format for JSON serialization
        if packet.get("raw_payload"):
            if isinstance(packet["raw_payload"], (bytes, memoryview)):
                packet["raw_payload"] = (
                    packet["raw_payload"].hex()
                    if hasattr(packet["raw_payload"], "hex")
                    else str(packet["raw_payload"])
                )
            elif isinstance(packet["raw_payload"], str):
                # Already converted to hex string, keep as is
                pass

        # Add derived fields
        from datetime import timezone, timedelta
        est = timezone(timedelta(hours=-5))  # EST is UTC-5
        packet["timestamp_str"] = datetime.fromtimestamp(packet["timestamp"], tz=est).strftime(
            "%Y-%m-%d %H:%M:%S EST"
        )
        packet["hop_count"] = (
...
def decode_packet_payload(packet: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        if decoded_payload.get("type") == "text_message":
            ...
            payload_info["text"] = (
                packet["raw_payload"] if packet["raw_payload"] else None
            )
            payload_info["decoded"] = False
        ...
        return {
            ...
            "decoded": False,
            "error": str(e),
            "text": packet["raw_payload"] if packet["raw_payload"] else None,
        }
...
def decode_protobuf_payload(packet: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        if not isinstance(portnum_name, str):
            return {
                "raw_bytes": raw_payload if raw_payload else None,
