...
class TraceroutePacket:
    ...
    def _parse_payload(self) -> RouteData:
        ...
        try:
            ...
            # Ensure raw_payload is bytes, not string or memoryview
            if isinstance(self.raw_payload, str):
                # If it's a hex string, decode it
                if self.raw_payload.startswith("0x") or all(
                    c in "0123456789abcdefABCDEF" for c in self.raw_payload
                ):
                    payload_bytes = bytes.fromhex(self.raw_payload)
                else:
                    # If it's a regular string, encode it as UTF-8
                    payload_bytes = self.raw_payload.encode("utf-8")
            elif isinstance(self.raw_payload, memoryview):
                # Convert memoryview to bytes
                payload_bytes = bytes(self.raw_payload)
            else:
                payload_bytes = self.raw_payload

            parsed_data = parse_traceroute_payload(payload_bytes)
